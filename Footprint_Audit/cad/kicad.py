...
def parse_schematic(sch_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols, exclude_libs, components, symbol_geometry):
    ...
    for element in sexpData: 
        ...
        if element[0] == Symbol('symbol'):
            newSymbol = parse_symbol(img_path, sym_img_fieldName, fp_img_fieldName, element)
...
def parse_all_schematics(root_sch_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols=[], exclude_libs=[]):
    ...
    def traverse(sch_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols, exclude_libs, components, symbol_geometries):
        ...
        sub_circuits = parse_schematic(sch_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols, exclude_libs, components, symbol_geometries)

        for sub_path in sub_circuits:
            abs_path = os.path.join(os.path.dirname(sch_path), sub_path)
            traverse(abs_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols, exclude_libs, components, symbol_geometries)

    traverse(root_sch_path, img_path, sym_img_fieldName, fp_img_fieldName, exclude_symbols, exclude_libs, components, symbol_geometries)
